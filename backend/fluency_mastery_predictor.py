...
class FluencyMasteryPredictor:
    def __init__(self, db):
        """Initialize predictor with MongoDB database"""
        self.db = db
    ...
    def predict_days_to_mastery(self, user_id: str) -> Dict:
        ...
        result = {
            ...
            'estimated_completion_date': (datetime.now() + timedelta(days=int(adjusted_prediction))).strftime('%Y-%m-%d'),
    ...
    def _baseline_prediction(self, features: Dict, total_trials: int) -> Dict:
        ...
        return {
            ...
            'estimated_completion_date': (datetime.now() + timedelta(days=int(predicted_days))).strftime('%Y-%m-%d'),
