...
class LanguageMasteryPredictor:
    def __init__(self, db, mode='receptive'):
        """
        ...
        self.mode = mode
        self.model = None
        self.model_path = os.path.join(os.path.dirname(__file__), 'models', f'language_{mode}_mastery_xgboost.pkl')
    ...
    def predict_days_to_mastery(self, user_id):
        ...
        if self.model is None:
            if not self.load_model():
                ...
                return self._baseline_prediction(progress)
    ...
    def _baseline_prediction(self, progress):
        ...
        return {
            ...
            'total_exercises': progress.get('total_exercises', 15),
            'current_accuracy': progress.get('accuracy', 0),
            'message': f'Estimated time to master {self.mode} language therapy: {predicted_days} days (baseline).',
