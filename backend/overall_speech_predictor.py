...
class OverallSpeechPredictor:
    ...
    def __init__(self, db):
        ...
        print("✅ Overall Speech Predictor initialized")
    ...
    def load_model(self):
        ...
        if os.path.exists(self.model_path):
            try:
                ...
                print(f"✅ Loaded overall speech improvement model from {self.model_path}")
                return True
    ...
    def predict_improvement(self, user_id):
        ...
        if total_trials < 3:
            return {
                ...
                'articulation_accuracy': 0,
                'fluency_accuracy': 0,
                'receptive_accuracy': 0,
                'expressive_accuracy': 0,
        ...
        confidence = self._calculate_confidence(total_trials, features)
    ...
    def _baseline_prediction(self, features):
        """Baseline prediction when model not available"""
        current_accuracy = features.get('overall_avg_accuracy', 0)
        total_trials = features.get('total_trials', 0)
        
        # Base weeks by current performance
        ...
        improvement_rate = ((1.0 - current_accuracy) / weeks) * 100 if weeks > 0 else 0
        
        return {
            'weeks_to_completion': weeks,
            'improvement_rate_per_week': round(improvement_rate, 2),
            'current_overall_accuracy': round(current_accuracy * 100, 1),
            'confidence': 0.5,
            'total_trials': total_trials,
            'therapy_types_active': features.get('therapy_types_active', 0),
            'articulation_accuracy': round(features.get('artic_avg_accuracy', 0) * 100, 1),
            'fluency_accuracy': round(features.get('fluency_accuracy', 0) * 100, 1),
            'receptive_accuracy': round(features.get('receptive_accuracy', 0) * 100, 1),
            'expressive_accuracy': round(features.get('expressive_accuracy', 0) * 100, 1),
            'message': f'Estimated {weeks} weeks to complete overall speech therapy (baseline estimate).',
            'note': 'Using baseline prediction (model not trained yet)'
        }
    
    def _calculate_confidence(self, total_trials, features):
        """Calculate confidence based on data quality"""
        confidence = 0.5
