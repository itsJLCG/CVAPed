...
function Header({ onLogout, onFacilityExit }) {
  const navigate = useNavigate();
  const location = useLocation();
  const { selectedCategory } = useTherapyCategory();

  const facilityMode = localStorage.getItem('facilityMode') === 'true';
  ...
  const handleFacilityExit = () => {
    if (onFacilityExit) onFacilityExit();
    navigate('/therapist');
  };
      ...
      <header className="app-header">
        <div className="app-header-container">
          <div className="app-logo-group">
            ...
            <span className="facility-mode-badge">Facility Mode</span>
          </div>
          <nav className="app-nav">
            <button onClick={handleFacilityExit} className="nav-btn facility-exit-btn">
              Exit Facility Mode
            </button>
          </nav>
        </div>
