...
export const generateTherapistReportsPdf = async ({
  reportTitle = 'Therapist Reports',
  reportDescription = '',
  ...
  filename = 'CVAPed_Therapist_Report',
}) => {
  const [{ default: jsPDF }, { default: autoTable }, logoDataUrl] = await Promise.all([
    import('jspdf'),
    import('jspdf-autotable'),
  ...
  addBrandedHeader(doc, reportTitle, logoDataUrl);
  ...
  const metaColW = (pageWidth - 2 * PAGE_MARGIN) / 3;
  metaItems.forEach((item, i) => {
    const x = PAGE_MARGIN + i * metaColW + metaColW / 2;
    if (i > 0) {
      doc.line(PAGE_MARGIN + i * metaColW, HEADER_HEIGHT + 3, PAGE_MARGIN + i * metaColW, HEADER_HEIGHT + META_BAR_HEIGHT - 3);
    }
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(5.5);
    doc.setTextColor(...C.textMuted);
    ...
    doc.setFontSize(7.5);
    doc.setTextColor(...C.textBody);
    doc.text(String(item.value || '—'), x, HEADER_HEIGHT + 11.5, { align: 'center' });
  });
  ...
  if (reportDescription) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(...C.textBody);
    const descriptionLines = doc.splitTextToSize(reportDescription, pageWidth - PAGE_MARGIN * 2);
    doc.text(descriptionLines, PAGE_MARGIN, yPos);
    yPos += descriptionLines.length * 4 + 4;
  }

  if (summaryStats.length > 0) {
    yPos = renderSectionLabel(doc, 'Summary', yPos);
    autoTable(doc, {
      startY: yPos,
      head: [['Metric', 'Value']],
      body: summaryStats.map((item) => [String(item.label || 'Metric'), String(item.value ?? '—')]),
      margin: { left: PAGE_MARGIN, right: PAGE_MARGIN },
      styles: {
        fontSize: 8,
        cellPadding: { top: 3, bottom: 3, left: 5, right: 5 },
      ...
      columnStyles: {
        0: { fontStyle: 'bold', textColor: C.textMuted },
  ...
  if (overallItems.length > 0) {
    yPos = renderSectionLabel(doc, `${categoryLabel} Overview`, yPos);
    autoTable(doc, {
      startY: yPos,
      head: [['Category', 'Patients', 'Percentage']],
      body: overallItems.map((item) => [
        String(item.label || item.range || item.gender || item.key || 'Unknown'),
        String(item.count ?? 0),
        `${item.percentage ?? 0}%`,
      ]),
      margin: { left: PAGE_MARGIN, right: PAGE_MARGIN },
  ...
  therapyPanels.forEach((panel, index) => {
    if (yPos > pageHeight - 70 && index > 0) {
      doc.addPage();
      addBrandedHeader(doc, reportTitle, logoDataUrl);
      yPos = HEADER_HEIGHT + 10;
    }

    yPos = renderSectionLabel(doc, panel.therapyLabel || panel.therapyType || 'Therapy', yPos);
    autoTable(doc, {
      startY: yPos,
      head: [['Category', 'Patients', 'Percentage']],
      body: (panel.items || []).map((item) => [
        String(item.label || item.range || item.gender || item.key || 'Unknown'),
        String(item.count ?? 0),
        `${item.percentage ?? 0}%`,
      ]),
  ...
  if (detectedProblems.length > 0) {
    ...
    yPos = renderSectionLabel(doc, 'Detected Problems by Gender', yPos);
    autoTable(doc, {
      startY: yPos,
      head: [['Problem', 'Male', 'Female', 'Other', 'Prefer Not To Say', 'Most Common']],
      body: detectedProblems.map((problem) => [
        String(problem.label || 'Unknown'),
        String(problem.countsByGender?.male ?? 0),
        String(problem.countsByGender?.female ?? 0),
        String(problem.countsByGender?.other ?? 0),
        String(problem.countsByGender?.['prefer-not-to-say'] ?? 0),
        String(problem.dominantGenderLabel || 'N/A'),
      ]),
      margin: { left: PAGE_MARGIN, right: PAGE_MARGIN },
      styles: {
        fontSize: 7.5,
      ...
      columnStyles: {
        0: { fontStyle: 'bold' },
        5: { textColor: C.primary, fontStyle: 'bold' },
      },
      theme: 'grid',
    });
  }
